...
export class ViewProjectComponent implements OnInit {
  ...
  deleteBid(){
    if(window.confirm('Do you want to delete the bid and set a fixed price?')){
      this.authpro.deleteBid(this.credential).subscribe()
      this.marked4 = false
      this.marked3 = true    
    }  
  ...
  deletePrice(){
    if(window.confirm('Do you want to delete the fixed price and start a bid?')){
    this.authpro.deletePrice(this.credentials).subscribe()
    this.credentials.payment = ''
    this.marked4 = true
    this.marked3 = false
    }
  }
